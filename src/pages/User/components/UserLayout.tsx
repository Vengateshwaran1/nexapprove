...
/* ─── Navigation Items ────────────────────────────────────────────────────── */
const MAIN_NAV = [
  { label: 'Home', icon: LayoutDashboard, to: '/user/dashboard' },
  { label: 'My Approvals', icon: CheckSquare, to: '/user/approvals' },
  { label: 'My Submissions', icon: Send, to: '/user/submissions' },
...
const UserLayout = () => {
  ...
  const userEmail = user?.email ?? 'user@organization.com';
    ...
    <div className="flex h-screen overflow-hidden bg-background text-foreground transition-colors duration-300">
      ...
      >
        ...
        <div className="flex items-center gap-3 px-6 py-5">
          ...
          <span className="ml-auto rounded-md bg-blue-500/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-widest text-blue-500">
            User
          </span>
        </div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="sa-topbar flex items-center gap-4 border-b border-border bg-background/80 px-4 py-3 backdrop-blur-md sm:px-6">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            {/* New Request */}
            <button
              className="hidden items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground shadow-md transition-all hover:bg-primary/90 hover:shadow-lg sm:inline-flex"
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="sa-user-menu flex items-center gap-2 rounded-xl px-2 py-1.5 transition-colors hover:bg-accent focus-visible:outline-none">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-linear-to-br from-blue-500 to-indigo-400 text-xs font-bold text-white shadow-md">
                    {initials}
                  </div>
              ...
              <DropdownMenuContent align="end" className="w-64 rounded-xl p-0">
                ...
                <div className="flex items-center gap-3 px-4 py-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-blue-500 to-indigo-400 text-sm font-bold text-white shadow-md">
                    {initials}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold">{fullName}</p>
                    <p className="truncate text-xs text-muted-foreground">{userEmail}</p>
                    <span className="mt-0.5 inline-block rounded-md bg-blue-500/10 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-blue-500">
                      User
                    </span>
                  </div>
                </div>
